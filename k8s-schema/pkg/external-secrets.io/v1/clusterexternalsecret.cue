...
#ClusterExternalSecret: {
	...
	spec?: {
		...
		externalSecretName?: strings.MaxRunes(
		...
		externalSecretSpec!: {
			...
			data?: [...{
				...
				secretKey!: strings.MaxRunes(
						253) & strings.MinRunes(
						1) & =~"^[-._a-zA-Z0-9]+$"

				// SourceRef allows you to override the source
				// from which the value will be pulled.
				sourceRef?: struct.MinFields(
					1) & struct.MaxFields(
					1) & {
				...
				rewrite?: [...struct.MinFields(
					...
					1) & {
						...
						merge?: {
							...
							// Used to define the policy when a key in the priority list does
						...
						// The resulting key will be the output of the template applied by
						// the operation.
						transform?: {
	...
	// ClusterExternalSecretStatus defines the observed state of
	// ClusterExternalSecret.
	status?: {
		conditions?: [...{
			message?: string
		...
		// ExternalSecretName is the name of the ExternalSecrets created
		// by the ClusterExternalSecret
		externalSecretName?: string

		// Failed namespaces are the namespaces that failed to apply an
		// ExternalSecret
		failedNamespaces?: [...{
			// Namespace is the namespace that failed when trying to apply an
			// ExternalSecret
			namespace!: string

			// Reason is why the ExternalSecret failed to apply to the
			// namespace
			reason?: string
		}]

		// ProvisionedNamespaces are the namespaces where the
