...
#Gateway: {
	...
	spec!: {
		...
		addresses?: list.MaxItems(16) & [...matchN(1, [{
			...
			type?: strings.MaxRunes(
				...
				1) & =~"^Hostname|IPAddress|NamedAddress|[a-z0-9]([-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*\\/[A-Za-z0-9\\/\\-._~%!$&'()*+,;=:]+$"
		...
		gatewayClassName!: strings.MaxRunes(
					253) & strings.MinRunes(
					1)
		...
		infrastructure?: {
			...
			labels?: struct.MaxFields(
				8) & {
					[string]: strings.MaxRunes(
							63) & strings.MinRunes(
							0) & =~"^(([A-Za-z0-9][-A-Za-z0-9_.]*)?[A-Za-z0-9])?$"
				}
	...
	status?: {
		// Addresses lists the network addresses that have been bound to
		// the
		// Gateway.
		//
		// This list may differ from the addresses provided in the spec
		// under some
		// conditions:
		//
		// * no addresses are specified, all addresses are dynamically
		// assigned
